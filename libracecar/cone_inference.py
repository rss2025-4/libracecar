...
from libracecar.plot import plot_ctx, plot_point, plot_style
...
class cone_location(eqx.Module):
    ...
    def plot(
        self: batched["cone_location", ...],
        style: plot_style = plot_style(),
    ):
        self = flatten_n_tree(self, len(self.loc.shape) - 1)
        return jax.vmap(lambda l: plot_point(l[0], l[1], style))(self.loc).plot()
...
class cone_dist(eqx.Module):
    ...
    def plot(
        ...
        cone_samples, _ = jax.vmap(lambda key: self.sample(rng_key=key))(
            random.split(rng_key, num=n_points)
        )
        return cone_samples.plot(style)
    ...
    def guide():
        d = cone_dist(
            ...
            numpyro_param(
                ...
                constraints.greater_than_eq(0.5),
            ),
        )
        _ = d.sample()
